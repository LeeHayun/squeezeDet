...
class SqueezeDet(ModelSkeleton):
  ...
  def _add_forward_graph(self):
    ...
    # Deconvolation
    fire_deconv10 = self._fire_deconv(
        'fire_deconv10', fire9, s1x1=64, e1x1=128, e3x3=128, factors=[1, 2],
        stddev=0.1)
    fire10_fuse = tf.add(fire_deconv10, fire5, name='fure10_fuse')

    fire_deconv11 = self._fire_deconv(
        'fire_deconv11', fire10_fuse, s1x1=32, e1x1=64, e3x3=64, factors=[1, 2],
        stddev=0.1)
    fire11_fuse = tf.add(fire_deconv11, fire3, name='fire11_fuse')

    fire_deconv12 = self._fire_deconv(
        'fire_deconv12', fire11_fuse, s1x1=16, e1x1=32, e3x3=32, factors=[1, 2],
        stddev=0.1)
    fire12_fuse = tf.add(fire_deconv12, conv1, name='fire12_fuse')

    fire_deconv13 = self._fire_deconv(
        'fire_deconv13', fire12_fuse, s1x1=16, e1x1=32, e3x3=32, factors=[1, 2],
        stddev=0.1)
    #fire13_fuse = tf.add(fire_deconv13, conv1_skip, name='fire13_fuse')

    #drop13 = tf.nn.dropout(fire13_fuse, self.keep_prob, name='drop13')
    drop13 = tf.nn.dropout(fire_deconv13, self.keep_prob, name='drop13')
    ...
    """
    self.lane_preds = self._conv_layer(
        'conv14', drop13, filters=2, size=3, stride=1,
        padding='SAME', xavier=False, relu=False, stddev=0.0001)
