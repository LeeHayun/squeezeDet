...
class ciss(imdb):
  def __init__(self, image_set, data_path, mc):
    imdb.__init__(self, 'ciss_'+image_set, mc)
    self._image_set = image_set
    self._data_root_path = data_path
    self._image_path = os.path.join(self._data_root_path, 'training', 'image')
    self._object_depth_label_path = os.path.join(self._data_root_path, 'training', 'depth')
    #self._depth_label_path = os.path.join(self._data_root_path, 'training', 'depth')
    self._lane_image_path = os.path.join(self._data_root_path, 'training', 'lane')
    self._classes = self.mc.CLASS_NAMES
    self._class_to_idx = dict(zip(self.classes, xrange(self.num_classes)))

    # a list of string indices of images in the directory
    self._image_idx = self._load_image_set_idx() 
    # a dict of image_idx -> [[cx, cy, w, h, cls_idx, depth]]. x,y,w,h are not divided by
    # the image width and height
    self._rois = self._load_ciss_object_depth_annotation()
  ...
  def _load_ciss_object_depth_annotation(self):
    ...
    for index in self._image_idx:
      filename = os.path.join(self._object_depth_label_path, index+'.txt')
      ...
      for line in lines:
        ...
        bboxes.append([x, y, w, h, cls, depth])
